...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
		...
		<meta name="Keywords" data-type="text" content="@if(isset($meta_keywords) && $meta_keywords!=''){{stripslashes($meta_keywords)}}@else{{config('global.META_KEYWORDS')}}@endif">
		<meta name="Description" data-type="text" content="@if(isset($meta_description) && $meta_description!=''){{stripslashes($meta_description)}}@else{{config('global.META_DESCRIPTION')}}@endif">
		...
		<!-- <script id="usercentrics-cmp" src="https://web.cmp.usercentrics.eu/ui/loader.js" data-ruleset-id="0X-_UORFfdip7G" async></script> -->
		...
		@php
            ...
            $commonVer = filemtime(config('global.SITE_STYLE_CORE_PATH').'common.css');
            $HeaderVer = filemtime(config('global.SITE_STYLE_CORE_PATH').'header-footer.css');
            ...
            $jqueryvalidateJSVer = filemtime(config('global.SITE_JS_CORE_PATH').'jquery.validate.js');
			...
			$SlimscrollJSVer = filemtime(config('global.SITE_JS_CORE_PATH').'jquery.slimscroll.js');
		...
		@if($CurrentRoute == 'home')
			<script src="https://cdn-widgetsrepository.yotpo.com/v1/loader/uIl5V6C_LVeCr5BT4bhDLQ" async></script>
		@endif
		...
		<link rel="stylesheet" type="text/css" media="all" href="{{ config('global.SITE_STYLE_CORE')}}common.css?ver={{$commonVer}}">
		<link rel="preload" as="style" href="{{ config('global.SITE_STYLE_CORE')}}header-footer.css?ver={{$HeaderVer}}" onload="this.rel='stylesheet'">
		...
		@if(request()->is('store/*') || (Session::has('sess_storeuserid') && Session::get('sess_storeuserid')!=''))
			...
			<link rel="stylesheet" type="text/css" media="all" href="{{ config('global.SITE_STYLE_CORE')}}pos-style.css?ver={{$posstyleVer}}">
    ...
    <body class="{{ $body_class ?? ''}}" data-test="{{Session::get('new')}}">
		...
		@if($CurrentRoute != 'billing' && $CurrentRoute != 'billing-shipping' && $CurrentRoute != 'billing-payment' && $CurrentRoute != 'order-receipt' && $CurrentRoute != 'phoneorder_payment_receipt' && $CurrentRoute != 'AmazonBilling' && $CurrentRoute != 'AmazonBillingFund' && $CurrentRoute != 'AmazonPhoneOrderCheckout' && $CurrentRoute !='billing' && $CurrentRoute != 'billing-shipping' && $CurrentRoute != 'billing-payment' && $CurrentRoute != 'temp_proddetails_code' && $CurrentRoute != 'order-receipts' && $CurrentRoute != 'store-payment')
		...
		{{-- @if(Session::has('Afterpay') && ($CurrentRoute == 'shoppingcart' || $CurrentRoute == 'billing' || $CurrentRoute == 'proddetails' || $CurrentRoute == 'proddetails_size' || $CurrentRoute == 'proddetails_code' )) --}}
		...
		@if($CurrentRoute == 'billing')
			...
			// 	$('#bill_address1, #ship_address1').each(function () {
			// 		const element = this; // DOM node
			...
			// 			fillAddressFields(place, element.id);
			// 		});
			// 	});
			// }
		...
		<script type="text/javascript" src="{{config('global.SITE_JS_CORE')}}jquery.validate.js?ver={{$jqueryvalidateJSVer}}"></script>
		<script type="text/javascript" src="{{config('global.SITE_JS_CORE')}}jquery.selectbox-0.2.min.js?ver={{$jqueryJSselectboxVer}}"></script>
		...
		@if(config('IsMobileCon') == "Yes")
			@if($CurrentRoute!="home" && $CurrentRoute!="proddetails" && $CurrentRoute!="proddetails_size" && $CurrentRoute!="proddetails_code" && $CurrentRoute!="billing" && $CurrentRoute!="billing-shipping" && $CurrentRoute!="billing-payment"  && $CurrentRoute!="order-receipt" && $CurrentRoute!="order-receipts")
				<script type="text/javascript" src="{{config('global.SITE_JS_CORE')}}slick.js?ver=565656569"></script>
			@endif
		@endif

		<script type="text/javascript" src="{{config('global.SITE_JS_CORE')}}modal.js?ver={{$ModalJSVer}}"></script>
		...
		<script type="text/javascript" src="{{config('global.SITE_JS_CORE')}}common_new.js?ver={{$CommonJSNewVer}}"></script>
		@else
		<script type="text/javascript" src="{{config('global.SITE_JS_CORE')}}common.js?ver={{$CommonJSVer}}"></script>
		@endif
		@if($CurrentRoute=="create-order")
		<script type="text/javascript" src="{{config('global.SITE_JS_CORE')}}poscheckout.js?ver={{$POSCheckoutJSNewVer}}"></script>
		@endif

		<script type="text/javascript" src="{{config('global.SITE_JS_CORE')}}popup.js?ver={{$PopupJSVer}}"></script>
		<script type="text/javascript" src="{{config('global.SITE_JS_CORE')}}smooth-scroll.js?ver={{$SmoothScrollJSVer}}"></script>
		<script type="text/javascript" src="{{config('global.SITE_JS_CORE')}}menu-new.js?ver={{$MenunewJSVer}}"></script>
		<script type="text/javascript" src="{{config('global.SITE_JS_CORE')}}jquery.slimscroll.js?ver={{$SlimscrollJSVer}}"></script>
		<script type="text/javascript" src="{{config('global.SITE_JS_CORE')}}head-bottom.js?ver={{$HeadBottomJSVer}}"></script>
		@if($CurrentRoute=="shoppingcart_new" || $CurrentRoute=="setcart_new" || $CurrentRoute=="getcarthtml_new" || $CurrentRoute=="getcartpartial_new" || $CurrentRoute=="billing_new" || $CurrentRoute=="billing_new-shipping" || $CurrentRoute=="billing_new-payment")
		<script type="text/javascript" src="{{config('global.SITE_JS_CORE')}}shoppingcart_new.js?ver={{$ShoppingcartJSNewVer}}"></script>
		@else
		<script type="text/javascript" src="{{config('global.SITE_JS_CORE')}}shoppingcart.js?ver={{$ShoppingcartJSVer}}"></script>
		@endif
		<script type="text/javascript" src="{{config('global.SITE_JS_CORE')}}newsletter.js?ver={{$NewsletterJSVer}}"></script>
		@if(isset($JSFILES))
